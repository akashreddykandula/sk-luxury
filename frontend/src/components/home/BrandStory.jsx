        ...
        <div className="grid md:grid-cols-2 gap-12 md:gap-20 items-center">
          ...
          >
            ...
            <div className="absolute bottom-8 -right-8 z-20 bg-emerald-950 p-5 shadow-luxury">
              ...
              <p className="font-sans text-[9px] text-white/60 tracking-[0.3em] uppercase mt-1">Since 2018</p>
            </div>
          </motion.div>
          ...
          >
            ...
            <h2 className="section-title text-left mb-5">Where Tradition Meets Modern Luxury</h2>
            <div className="flex items-center gap-3 mb-6">
              <div className="w-10 h-px bg-gold" />
              <div className="w-2 h-2 rotate-45 bg-gold" />
            </div>
            <p className="font-sans text-luxury-muted leading-relaxed mb-4">
              Founded in 2018 in the heart of Hyderabad, SK Luxury was born from a deep passion for artisanal
            ...
            <div className="grid grid-cols-2 gap-5 mb-8">
              {stats.map(s => (
                <div key={s.label} className="border-l-2 border-gold pl-4">
                  <p className="font-display text-3xl text-emerald-900">{s.value}</p>
                  <p className="font-sans text-xs text-luxury-muted tracking-widest uppercase mt-0.5">{s.label}</p>
                </div>
              ))}
            </div>

            <Link to="/about" className="btn-luxury group">
              Our Full Story <FiArrowRight size={15} className="group-hover:translate-x-1 transition-transform" />
            </Link>
          </motion.div>
        </div>
      </div>
